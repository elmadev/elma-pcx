...
export class PCX {
  ...
  constructor(buffer: Uint8Array | Buffer) {
    ...
    const dataView = new DataView(this.byteView.buffer, this.byteView.byteOffset, this.byteView.byteLength);
    ...
    this.header = {
      ...
      bpp: this.byteView[3],
      xmin: dataView.getInt16(4, true),
      ymin: dataView.getInt16(6, true),
      xmax: dataView.getInt16(8, true),
      ymax: dataView.getInt16(10, true),
      bitplanes: this.byteView[65],
      bpr: dataView.getInt16(66, true),
    };
   ...
   * @param transparency - An enum value from Transparency
   * @returns {number} The palette id that should be transparent
   */
  private _getTransparencyPaletteId(transparency: Transparency) {
    switch (transparency) {
   ...
   * @param palette - A palette from getPalette()
   * @param transparentPaletteId - A palette id from _getTransparencyPaletteId()
   * @returns {Uint8ClampedArray} Flattened RGBA pixel array of size 4*width*height
   */
  private _colorize(palette: Uint8Array, transparentPaletteId: number) {
   ...
   * @param palette - A palette from getPalette()
   * @param transparency - A transparency option from Transparency
   * @returns {Uint8ClampedArray} Flattened RGBA pixel array of size 4*width*height
   */
  public getImage(palette: Uint8Array, transparency: Transparency) {
 ...
 * @param pixels - A bitmap with palette ids of size width*height
 * @param width - Image width
 * @param height - Image height
 * @param palette - A 768-byte palette as provided by getPalette()
 * @returns {Uint8Array} file data buffer
 */
export const writePCX = (
  pixels: Uint8Array,
  width: number,
  height: number,
